...
| --- | --- | --- |
| `EmailDomain` | Domain appended to Windows usernames for auto-registration | `yourcompany.com` |
| `SmtpSenderAddress` | From address on all outbound emails | `noreply@yourcompany.com` |
| `SmtpSenderDisplayName` | Display name on outbound emails | `Laptop/PC Agreement System` |
...
1. Create the SQL Server database and run the schema script.
2. Update `Web.config` with your connection string, SMTP settings, and `<appSettings>` values.
3. Publish to IIS with Windows Authentication enabled and Anonymous Authentication disabled.
